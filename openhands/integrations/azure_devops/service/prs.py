...
class AzureDevOpsPRsMixin(AzureDevOpsMixinBase):
    ...
    async def add_pr_inline_thread(
        self,
        repository: str,
        pr_number: int,
        comment_text: str,
        ...
        """Create a new thread with file/line context in an Azure DevOps pull request.

        This creates an inline comment positioned on a specific file and line range
        in the PR diff. Uses the ``threadContext`` payload to position the comment.
        ...
        Args:
            ...
            pr_number: The pull request number
            comment_text: The comment text to post
            file_path: Path to the file (must have a leading '/')
            line_start: 1-based start line number in the right (new) side of the diff
            line_end: Optional 1-based end line number (defaults to line_start)
            status: Thread status ('active', 'fixed', 'wontFix', 'closed', 'byDesign', 'pending')
        ...
        # Ensure file_path has a leading '/'
        ...
        logger.info(
            f'Created inline PR thread on {file_path}:{line_start} in {repository}#{pr_number}'
        )
        return response
