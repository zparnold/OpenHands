...
async def test_validate_llm_with_mock(test_client):
    ...
    # Mock the LLM class at multiple levels to avoid actual API calls
    with (
        patch('openhands.llm.llm.LLM') as mock_llm_class,
        patch('openhands.server.routes.settings.LLM') as mock_settings_llm_class,
    ):
        ...
        assert mock_settings_llm_class.called or mock_llm_class.called
